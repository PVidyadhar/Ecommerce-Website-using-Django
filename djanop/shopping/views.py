...
def index(request):
	...
	return render(request,'shopping/indexshopping.html',params)
...
def checkout(request):
	if request.method=="POST":
		...
		phone=request.POST.get('phone','')
		order=Orders(items_json=items_json,name=name,email=email,address=address,addressline2=addressline2,city=city,state=state,zip_code=zip_code,phone=phone)
		order.save()
